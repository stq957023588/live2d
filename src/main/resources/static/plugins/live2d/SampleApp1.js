...
function initL2dCanvas(canvasId)
{
    ...
    if(this.canvas.addEventListener) {
        ...
        this.canvas.addEventListener("keypress",mouseEvent,false)
    }
 ...
 * 更改模型
 */
function changeModel(btnChange)
{
    var originalTextContent;
    if(btnChange){
        originalTextContent=btnChange.textContent
    ...
    this.live2DMgr.changeModel(this.gl,function (){
        if(btnChange){
            btnChange.removeAttribute("disabled");
            btnChange.classList.remove("inactive")
            btnChange.textContent = originalTextContent;
        }
    });
}
